...
import {
  ArrowLeft, Building2, Search, Printer, MapPin,
...
function guessEquipmentType(instrumento: string): EquipmentType | null {
  ...
  if (name.includes("vazão") || name.includes("vazao")) return "flowmeter";
  if (name.includes("ph") || name.includes("phmetro")) return "ph-meter";
  if (name.includes("condutiv")) return "conductivity-meter";
  if (name.includes("espectro") || name.includes("fotôm") || name.includes("fotom")) return "spectrophotometer-pc03";
  if (name.includes("oxím") || name.includes("oxim")) return "oximeter";
  if (name.includes("fluxo laminar")) return "laminar-flow-hood";
  if (name.includes("cape")) return "capela";
  ...
  if (name.includes("torque") || name.includes("torquím")) return "torque";
  if (name.includes("cronôm") || name.includes("cronom") || name.includes("temporizador")) return "timer";
  if (name.includes("lux")) return "luxmeter";
  if (name.includes("micrôm") || name.includes("microm")) return "micrometro";
  if (name.includes("relógio comparador") || name.includes("relogio comparador")) return "relogio-comparador";
  if (name.includes("relógio apalpador") || name.includes("relogio apalpador")) return "relogio-apalpador";
  if (name.includes("paquím") || name.includes("paquim")) return "paquimetro";
  if (name.includes("trena")) return "trena";
  if (name.includes("rugos")) return "rugosimetro";
  if (name.includes("régua") || name.includes("regua")) return "regua-graduada";
  if (name.includes("espessura")) return "espessura";
  if (name.includes("dinamôm") || name.includes("dinamom")) return "dinamometro";
  if (name.includes("dureza") || name.includes("durôm") || name.includes("durom")) return "dureza";
  if (name.includes("megôm") || name.includes("megom")) return "megometro";
  if (name.includes("amperím") || name.includes("amperim") || name.includes("alicate")) return "alicate-amperimetro";
  if (name.includes("turbid")) return "turbidimetro";
  if (name.includes("densím") || name.includes("densim")) return "densimetro";
  if (name.includes("circôm") || name.includes("circom")) return "circometro";
  return null;
}
  ...
  return (
    <div className="h-screen flex flex-col bg-neutral-50">
      <div className="h-14 bg-neutral-900 border-b border-neutral-800 flex items-center px-4 gap-3 flex-shrink-0">
        ...
        <Button onClick={handlePrint} disabled={selected.size === 0} className="ml-auto gap-2 bg-orange-500 hover:bg-orange-600 disabled:opacity-50">
      ...
      <div className="bg-white border-b border-neutral-200 px-6 py-3 flex items-center gap-3 flex-shrink-0">
        <div className="relative flex-1 max-w-md">
          ...
          <Input placeholder="Buscar instrumento, código ou local..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-9" />
        </div>
        ...
        {loading ? (
          ...
          <div className="space-y-2 max-w-4xl mx-auto">
            {filtered.map((inst) => {
              ...
              return (
                <div key={inst.id} className={`bg-white rounded-xl border-2 transition-all overflow-hidden ${isSelected ? "border-orange-400 shadow-md shadow-orange-100" : "border-neutral-200 hover:border-neutral-300"}`}>
                  <div className="p-3 flex items-center gap-3">
                    <div className="cursor-pointer flex-shrink-0" onClick={() => toggleSelect(inst.id)}>
                      <div className={`w-5 h-5 rounded border-2 flex items-center justify-center transition-colors ${isSelected ? "border-orange-500 bg-orange-500" : "border-neutral-300"}`}>
                        {isSelected && <span className="text-white text-xs font-bold">✓</span>}
                      </div>
                    </div>
                    <div className="flex-1 min-w-0 cursor-pointer" onClick={() => toggleSelect(inst.id)}>
                      <div className="flex items-center gap-2 flex-wrap">
                        <span className="font-bold text-sm text-neutral-900">{inst.instrumento}</span>
                        {inst.codigo && <span className="text-xs bg-neutral-100 text-neutral-600 px-2 py-0.5 rounded-full font-mono">{inst.codigo}</span>}
                        {equipType && <span className="text-xs bg-orange-50 text-orange-600 px-2 py-0.5 rounded-full border border-orange-200">{getEquipmentTitle(equipType)}</span>}
                      </div>
                      <div className="flex items-center gap-3 mt-0.5 text-xs text-neutral-500">
                        {inst.faixa && <span>{inst.faixa}</span>}
                        {inst.local_utilizacao && inst.local_utilizacao !== "-" && (
                          <span className="flex items-center gap-1"><MapPin className="w-3 h-3" />{inst.local_utilizacao}</span>
                        )}
                      </div>
                    </div>
                    <div className="flex items-center gap-2 flex-shrink-0">
                      {inst.status && <span className={`text-xs px-2 py-0.5 rounded-full border font-medium ${getStatusColor(inst.status)}`}>{inst.status}</span>}
                      <button onClick={() => setExpandedInst(isExpanded ? null : inst.id)} className="p-1 hover:bg-neutral-100 rounded">
