...
export function TimerCalibrationForm({ data }: Props) {
  const F = ({ v }: { v?: string | null }) => (
    <span className="flex-1 border-b border-black min-w-0 px-0.5">{v || ""}</span>
  );

  const cronRows = [["10.0"],["100.0"],["1200.0"],["2400.0"],["3600.0"],["8600.0"]];
  const timerRows = Array(5).fill(null);
  ...
  return (
    <div className="print-form p-4 max-w-[210mm] mx-auto text-[9px] leading-tight">
      <h1 className="text-center font-bold text-xs mb-2 uppercase tracking-wide">
        Registro de Calibração de Tempo
      </h1>
      ...
      <div className="mb-2">
        <h4 className="font-bold text-[8px] mb-1">Medições (Cronômetro)</h4>
        <table className="print-table text-[7px] w-full mb-2">
          <thead>
            <tr><th>Testado (s)</th><th>Leitura 1</th><th>Leitura 2</th><th>Leitura 3</th></tr>
          </thead>
          <tbody>
            {cronRows.map(([v], i) => <tr key={i}><td className="h-4">{v}</td><td></td><td></td><td></td></tr>)}
          </tbody>
        </table>

        <h4 className="font-bold text-[8px] mb-1">Medições (Temporizador)</h4>
        <table className="print-table text-[7px] w-full">
          <thead>
            <tr><th>Testado (s)</th><th>Leitura 1</th><th>Leitura 2</th><th>Leitura 3</th></tr>
          </thead>
          <tbody>
            {timerRows.map((_, i) => <tr key={i}><td className="h-4"></td><td></td><td></td><td></td></tr>)}
          </tbody>
        </table>
      </div>
      ...
      <div className="mb-1">
        ...
        <div className="grid grid-cols-2 gap-x-2 text-[7px]">
          {["IP 06 - Temporizador - 0 a 86400s","IP 07 - Temporizador - 0 a 86400s","IP 119 - Cronômetro - 0 a 86400s"].map(p => (
